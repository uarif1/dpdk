...
static int send_vhost_reply(struct virtio_net *dev, struct vhu_msg_context *ctx);
			...
			struct vhu_msg_context *ctx)
{
	...
	ret = dev->trans_ops->set_slave_req_fd(dev, ctx);
...
static int
send_vhost_reply(struct virtio_net *dev, struct vhu_msg_context *ctx)
{
	if (!ctx)
		return 0;
	...
	return dev->trans_ops->send_reply(dev, ctx);
}
...
int
vhost_user_msg_handler(int vid, const struct vhu_msg_context *ctx_)
{
	struct virtio_net *dev;
	struct vhu_msg_context ctx = *ctx_;
	struct rte_vdpa_device *vdpa_dev;
	int ret;
	...
	if (ctx.msg.request.master >= VHOST_USER_MAX) {
	...
	if (request > VHOST_USER_NONE && request < VHOST_USER_MAX) {
		if (!vhost_message_handlers[request])
			goto skip_to_post_handle;
		ret = vhost_message_handlers[request](&dev, &ctx);
...
vhost_user_iotlb_miss(struct virtio_net *dev, uint64_t iova, uint8_t perm)
{
	...
	ret = dev->trans_ops->send_slave_req(dev, &ctx);
	if (ret < 0) {
...
vhost_user_slave_config_change(struct virtio_net *dev, bool need_reply)
{
	...
	return dev->trans_ops->process_slave_message_reply(dev, &ctx);
}
